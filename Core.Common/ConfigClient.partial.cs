...
namespace Core.Services
{
    public static partial class Config {
        public partial class ConfigClient : IConfigClient
        {
            ...
            {
                return GetSetting(new Key {Value = key}, new CallOptions(headers, deadline, cancellationToken));
            }
            ...
            public ConfigBlob GetGroupConfig(string groupName, Metadata headers = null, DateTime? deadline = null, CancellationToken cancellationToken = default)
            {
                return GetGroupConfig(new Key {Value = groupName}, new CallOptions(headers, deadline, cancellationToken));
            }

            public ConfigBlob GetGroupConfig(string groupName, CallOptions options)
            {
                return GetGroupConfig(new Key {Value = groupName}, options);
            }

            public AsyncUnaryCall<ConfigBlob> GetGroupConfigAsync(string groupName, Metadata headers = null, DateTime? deadline = null, CancellationToken cancellationToken = default)
            {
                return GetGroupConfigAsync(new Key {Value = groupName}, new CallOptions(headers, deadline, cancellationToken));
            }

            public AsyncUnaryCall<ConfigBlob> GetGroupConfigAsync(string groupName, CallOptions options)
            {
                return GetGroupConfigAsync(new Key {Value = groupName}, options);
            }

            public ConfigBlob GetUserConfig(Metadata headers = null, DateTime? deadline = null, CancellationToken cancellationToken = default)
            {
                return GetUserConfig(new Empty(), new CallOptions(headers, deadline, cancellationToken));
            }

            public ConfigBlob GetUserConfig(CallOptions options)
            {
                return GetUserConfig(new Empty(), options);
            }

            public AsyncUnaryCall<ConfigBlob> GetUserConfigAsync(Metadata headers = null, DateTime? deadline = null, CancellationToken cancellationToken = default)
            {
                return GetUserConfigAsync(new Empty(), new CallOptions(headers, deadline, cancellationToken));
            }

            public AsyncUnaryCall<ConfigBlob> GetUserConfigAsync(CallOptions options)
            {
